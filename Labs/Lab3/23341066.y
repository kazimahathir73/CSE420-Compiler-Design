...
symbol_table* table = new symbol_table(10);
...
string var_type;
string array_name;
vector<string> array_variable_choice;
...
func_definition : type_specifier ID LPAREN parameter_list RPAREN enter compound_statement
		{	
			...
			ret_type = $1->getname();
			outlog<< ret_type <<  "    " << $2->getname() <<  "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl<< endl;
			function_name = $2->getname();

			symbol_info* to_insert = new symbol_info(  function_name, "func_def"  );
			to_insert->set_ret_type(ret_type);
			to_insert->set_param_num(param_number);
			to_insert->set_param_list(param_list);
		...
		| type_specifier ID LPAREN RPAREN enter compound_statement
		{
...
enter : {
	...
	outlog<< "New ScopeTable with ID " << to_string(  table->get_id() ) << " created " << endl <<endl;
	


	for (int i = 0; i< param_list.size() ;  i = i+2   ){
		symbol_info* to_insert = new symbol_info( param_list[i+1] , "variable"    );

		to_insert->set_var_type(param_list[i]);
...
parameter_list : parameter_list COMMA type_specifier ID
		{
			...
			param_list.push_back($4->getname());
		...
		| parameter_list COMMA type_specifier 
		{
 		...
 		| type_specifier ID
 		{
			...
			param_list.push_back($2->getname());
...
compound_statement : LCURL statements RCURL
			{ 
                ...
                // Note that function parameters should be in the current sco
...
var_declaration : type_specifier declaration_list SEMICOLON
		 {  
			...
			//outlog<< to_string(var_list.size())  <<  "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl<< endl;
			
			
			for (int i = 0  ;  i< var_list.size() ; i++){
				symbol_info* to_insert = new symbol_info( var_list[i] , array_variable_choice[i]   );
				to_insert->set_var_type($1->getname());
				table->insert(to_insert);
			}
			var_list.clear();
			array_variable_choice.clear();
...
declaration_list : declaration_list COMMA ID
		  {
			...
			array_variable_choice.push_back("variable");
 		  }
 		  | declaration_list COMMA ID LTHIRD CONST_INT RTHIRD //array after some declaration
 		  {
			...
			var_list.push_back($3->getname());
			array_variable_choice.push_back("array");
...
statement : var_declaration
	  ...
	  | FOR LPAREN expression_statement expression_statement expression RPAREN enter_compound_for statement
	  {
	  ...
	  | IF LPAREN expression RPAREN enter_compound_if statement %prec LOWER_THAN_ELSE
	  {
	  ...
	  | IF LPAREN expression RPAREN enter_compound_if  statement ELSE statement
	  {
	  ...
	  | WHILE LPAREN expression RPAREN enter_compound_while statement
	  {
